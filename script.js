...
$('table tr td').click (function(){
    if($(this).text()== "" && play){
        ...
        if( (move%2)==1){
            ...
            $(this).append("O");
            $(this).css('color', '#085a4f');
        }
        move++;
        if (findWinnwer() != -1 && findWinnwer()!= ""){
            if(findWinnwer() == "X"){
        ...
        } else if (move == 10 && findWinnwer() == -1) {
            $('body').append('<div class = "winner"><span> No Winner</span>Game Draw</div>');
            $('body').append('<button onclick="location.reload()">Reset</button>');
            $('.winner').css('background-color', '#e85a4f');
            $('button').css('color', '#e85a4f');
        }
